...
router.patch(
  ...
  (req, res) => {
    res.status(501).json({
      success: false,
      message:
        'Modification partielle bibliotheque - Fonctionnalite a implementer',
    });
  }
);
...
// POST /api/libraries/:id/add-book - Ajouter un livre à une bibliothèque (collection)
router.post('/:id/add-book',
    authenticateToken,
    // validateParams(libraryParamsSchema),
    // validateBody(addBookToLibrarySchema),
    addBookToLibrary
);
