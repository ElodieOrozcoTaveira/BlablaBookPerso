...
router.post('/login', loginRateLimit, async (req: TypedRequest<LoginRequest>, res: Response) => {
    try {
        ...
        // Détruire toute session existante (une seule session par user)
        if (req.session) {
            req.session.destroy((err) => {
                if (err) console.warn('Error destroying previous session:', err);
            });
        }
        ...
        console.log(`[AUTH] User logged in: ${user.email} (ID: ${user.id_user})`);
