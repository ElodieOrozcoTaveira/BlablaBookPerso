 ...
 * Body: { name: string }
 */
router.post("/", requireAuth, async (req: SessionRequest, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Erreur interne du serveur lors de la création de la bibliothèque",
...
router.get("/", requireAuth, async (req: SessionRequest, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Erreur interne du serveur lors de la récupération des bibliothèques",
      error: process.env.NODE_ENV === "development" ? error : undefined,
    });
  }
});
...
router.get("/:id", requireAuth, async (req: SessionRequest, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Erreur interne du serveur lors de la récupération de la bibliothèque",
      error: process.env.NODE_ENV === "development" ? error : undefined,
    });
  }
});
 ...
 * Body: { name: string }
 */
router.put("/:id", requireAuth, async (req: SessionRequest, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Erreur interne du serveur lors de la mise à jour de la bibliothèque",
      error: process.env.NODE_ENV === "development" ? error : undefined,
    });
  }
});
  ...
  async (req: SessionRequest, res: Response) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "Erreur interne du serveur lors de la suppression de la bibliothèque",
        error: process.env.NODE_ENV === "development" ? error : undefined,
      });
    }
  }
