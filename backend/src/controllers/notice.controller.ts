...
import { BookActionService } from '../services/book-action.service.js'; // conserve pour compat tests si injecté ailleurs
...
// req.params deja valide par middleware
        ...
        const notice = await Notice.findByPk(id, {
            ...
            include: [
                {
                    association: 'NoticeBelongsToUser',
                    attributes: ['id_user', 'username', 'avatar_url']
                },
                {
                    association: 'NoticeBelongsToBook',
...
export const updateNotice = async (
    ...
    try {
        ...
        if (!notice) {
            ...
            if ((notice.dataValues?.id_user || notice.get('id_user')) !== req.user.id_user && !(req.user as any).is_admin) {
