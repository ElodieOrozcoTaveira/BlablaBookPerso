...
export class BookController {
     ...
     */
    static async searchOpenLibrary(req, res) {
        try {
     ...
     */
    static async getOpenLibraryDetails(req, res) {
        try {
     ...
     * Recherche par ISBN
     */
    static async searchByISBN(req, res) {
        try {
     ...
     */
    static async saveBookFromOpenLibrary(req, res) {
        try {
            ...
            const openLibraryBook = await OpenLibraryService.getBookDetails(openLibraryId);
            // Sauvegarder en BDD via BookService
            const savedBook = await BookService.saveFromOpenLibrary(sessionUser.id, openLibraryBook, status);
            res.status(201).json({
                success: true,
                data: savedBook,
    ...
    static async updateBookStatus(req, res) {
        try {
            ...
            await sequelize.query(`UPDATE book_library 
     ...
     */
    static async addBookToLibrary(req, res) {
        try {
            const userId = req.session?.user?.id;
            ...
            const existingRelation = await sequelize.query(`SELECT * FROM book_library 
         WHERE id_library = :libraryId AND id_book = :bookId`, {
                replacements: {
                    libraryId: library.get("id"),
                    bookId: book.get("id"),
            ...
            if (existingRelation[0] && existingRelation[0].length > 0) {
                res.status(409).json({
                    success: false,
            ...
            await sequelize.query(`INSERT INTO book_library (id_library, id_book, status, created_at, updated_at)
