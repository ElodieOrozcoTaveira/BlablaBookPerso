...
export class LibraryController {
    ...
    static async getUserLibraries(req, res) {
        try {
            ...
            if (search && typeof search === "string") {
                libraries = await LibraryService.searchLibraries(sessionUser.id, search);
            }
            else {
    ...
    static async getLibraryById(req, res) {
        try {
            ...
            const library = await LibraryService.getLibraryById(libraryId, sessionUser.id);
            if (!library) {
                res.status(404).json({
                    success: false,
    ...
    static async updateLibrary(req, res) {
        try {
            ...
            const updatedLibrary = await LibraryService.updateLibrary(libraryId, sessionUser.id, { name });
            if (!updatedLibrary) {
                res.status(404).json({
                    success: false,
                    message: "Bibliothèque non trouvée",
    ...
    static async deleteLibrary(req, res) {
        try {
            ...
            const deleted = await LibraryService.deleteLibrary(libraryId, sessionUser.id);
            if (!deleted) {
                res.status(404).json({
                    success: false,
                    message: "Bibliothèque non trouvée",
