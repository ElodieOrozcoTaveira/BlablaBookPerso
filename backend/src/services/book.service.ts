...
export class BookService {
    ...
    private async importAndAssociateAuthors(openLibBook: any, bookId: number): Promise<void> {
        try {
            // Je traite l'importation et l'association des auteurs pour ce livre
            
            if (!openLibBook.author_name || !openLibBook.author_key) {
                // Je verifie qu'il y a des auteurs a importer
                return;
            }
            ...
            for (let i = 0; i < openLibBook.author_name.length; i++) {
                ...
                if (authorName && authorKey) {
                    try {
                        ...
                        // Je garde une trace de l'auteur associe pour le debug si necessaire
                    } catch (error) {
            ...
            if (authorIds.length > 0) {
                // Je prepare l'association des auteurs au livre
                ...
                for (const authorId of authorIds) {
                    try {
                        ...
                        // Je confirme la creation de l'association
                    } catch (error) {
                        console.error(`Erreur association livre ${bookId} <-> auteur ${authorId}:`, error);
                    }
                }
                
                // Je confirme que toutes les relations ont ete ajoutees
            } else {
                // Je note qu'aucun auteur n'a ete trouve a associer
            }
