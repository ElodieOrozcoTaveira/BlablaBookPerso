...
// Types pour les données de bibliothèque
...
export class LibraryService {
   ...
   */
  static async createLibrary(libraryData: LibraryCreate): Promise<LibraryData> {
    try {
      // 1. Validation des données
      if (!libraryData.name || libraryData.name.trim().length < 2) {
        throw new LibraryError(
          "Le nom de la bibliothèque doit contenir au moins 2 caractères",
          "INVALID_NAME",
          400
        );
      }
      ...
      if (existingLibrary) {
        throw new LibraryError(
          "Une bibliothèque avec ce nom existe déjà",
          "LIBRARY_NAME_EXISTS",
          409
        );
      }
   ...
   */
  static async getUserLibraries(userId: number): Promise<LibraryData[]> {
    try {
  ...
  static async updateLibrary(
    ...
    try {
      ...
      if (!library) {
        throw new LibraryError(
          "Bibliothèque non trouvée ou accès non autorisé",
          "LIBRARY_NOT_FOUND",
          404
        );
      }
  ...
  static async searchLibraries(
    ...
    try {
      ...
      throw new LibraryError(
        "Erreur lors de la recherche de bibliothèques",
        "SEARCH_ERROR",
        500
      );
    }
   ...
   */
  static async getUserLibraryStats(userId: number): Promise<{
