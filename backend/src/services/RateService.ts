...
export class RateService {
   ...
   */
  static async createRate(rateData: CreateRateData): Promise<RateData> {
    try {
   ...
   */
  static async getAllRates(filters: RateSearchFilters = {}): Promise<{
    rates: RateData[];
    total: number;
    page: number;
    totalPages: number;
    ...
    try {
      ...
      if (bookId) whereCondition.id_book = bookId;
      if (readingListId) whereCondition.id_reading_list = readingListId;

      if (minRate || maxRate) {
        whereCondition.rate = {};
        if (minRate) whereCondition.rate[Op.gte] = minRate;
        if (maxRate) whereCondition.rate[Op.lte] = maxRate;
      }
      ...
      const ratesList: RateData[] = rates.map((rate) => ({
      ...
      let averageRate: number | undefined;
      ...
      if (averageRate !== undefined) {
        (result as any).averageRate = averageRate;
      }
   ...
   */
  static async getRateById(rateId: number): Promise<RateData | null> {
    try {
   ...
   */
  static async getUserRateForBook(
    userId: number,
    bookId: number
   ...
   * Supprimer une note
   */
  static async deleteRate(rateId: number): Promise<void> {
    try {
      const rate = await Rate.findByPk(rateId);
      if (!rate) {
        throw new RateError("Note non trouvée", "RATE_NOT_FOUND", 404);
   ...
   */
  static async getBookAverageRate(bookId: number): Promise<{
    average: number;
    totalRates: number;
    ...
    try {
      ...
      rateValues.forEach((rate) => {
        distribution[rate as keyof typeof distribution]++;
      });
  ...
  static async getRateStats(): Promise<{
    totalRates: number;
    averageRate: number;
    distributionGlobal: { [key: number]: number };
    topRatedBooks: any[];
  }> {
    try {
      ...
      rateValues.forEach((rate) => {
        distributionGlobal[rate as keyof typeof distributionGlobal]++;
      });

      // TODO: Implémenter topRatedBooks avec jointure sur la table BOOK
      const topRatedBooks: any[] = [];
   ...
   */
  static async rateExists(rateId: number): Promise<boolean> {
    try {
      const rate = await Rate.findByPk(rateId);
      return !!rate;
    } catch (error) {
      throw new RateError(
        "Erreur lors de la vérification de la note",
        "CHECK_RATE_ERROR",
        500
      );
    }
   ...
   */
  static async getRecentRates(limit: number = 10): Promise<RateData[]> {
    try {
      const rates = await Rate.findAll({
        limit: Number(limit),
        order: [["published_at", "DESC"]],
