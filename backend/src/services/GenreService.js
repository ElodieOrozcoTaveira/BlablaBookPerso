...
export class GenreService {
    ...
    static async createGenre(genreData) {
        try {
            ...
            if (!genreData.name || genreData.name.trim().length === 0) {
                throw new GenreError("Le nom du genre est requis", "GENRE_NAME_REQUIRED", 400);
            }
            ...
            if (existingGenre) {
                throw new GenreError("Un genre avec ce nom existe déjà", "GENRE_ALREADY_EXISTS", 409);
            }
            // Créer le genre
            ...
            throw new GenreError("Erreur lors de la création du genre", "CREATE_GENRE_ERROR", 500);
        }
    }
    /**
     * Récupérer tous les genres avec filtres optionnels
     */
    static async getAllGenres(filters = {}) {
        try {
            ...
            throw new GenreError("Erreur lors de la récupération des genres", "GET_GENRES_ERROR", 500);
        }
    }
    /**
    ...
    static async getGenreById(genreId) {
        try {
            ...
            throw new GenreError("Erreur lors de la récupération du genre", "GET_GENRE_ERROR", 500);
        }
    }
    /**
    ...
    static async updateGenre(genreId, updateData) {
        try {
            ...
            if (updateData.name !== undefined &&
                ...
                throw new GenreError("Le nom du genre ne peut pas être vide", "GENRE_NAME_REQUIRED", 400);
            }
            ...
            throw new GenreError("Erreur lors de la mise à jour du genre", "UPDATE_GENRE_ERROR", 500);
        }
    }
    /**
     * Supprimer un genre
    ...
    static async deleteGenre(genreId) {
        try {
            ...
            throw new GenreError("Erreur lors de la suppression du genre", "DELETE_GENRE_ERROR", 500);
        }
    }
    /**
     * Rechercher des genres par nom
     */
    static async searchGenres(searchTerm, limit = 10) {
        try {
            ...
            throw new GenreError("Erreur lors de la recherche de genres", "SEARCH_GENRES_ERROR", 500);
        }
    }
    /**
    ...
    static async getGenreStats() {
        try {
            ...
            throw new GenreError("Erreur lors de la récupération des statistiques", "GET_STATS_ERROR", 500);
        }
    }
    /**
     * Vérifier si un genre existe
     */
    static async genreExists(genreId) {
        try {
            const genre = await Genre.findByPk(genreId);
            return !!genre;
        }
        catch (error) {
            throw new GenreError("Erreur lors de la vérification du genre", "CHECK_GENRE_ERROR", 500);
        }
    }
    /**
    ...
    static async getPopularGenres(limit = 10) {
        try {
            ...
            throw new GenreError("Erreur lors de la récupération des genres populaires", "GET_POPULAR_GENRES_ERROR", 500);
        }
    }
    /**
    ...
    static async getGenresAlphabetically() {
        try {
            ...
            throw new GenreError("Erreur lors de la récupération des genres", "GET_ALPHABETICAL_GENRES_ERROR", 500);
        }
    }
}
