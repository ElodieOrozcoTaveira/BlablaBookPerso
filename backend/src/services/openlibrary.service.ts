...
export class OpenLibraryService {
    ...
    async downloadAndProcessCover(coverId: number, bookId: number): Promise<string | null> {
        try {
            ...
            // Traiter l'image (creer toutes les tailles)
            const processedImages = await imageService.processImage(buffer, 'book', bookId);
    ...
    async downloadAndProcessAuthorPhoto(photoId: number, authorId: number): Promise<string | null> {
        try {
            ...
            // Traiter la photo d'auteur
            const processedImages = await imageService.processImage(buffer, 'author', authorId);
