...
export class NoticeService {
   ...
   */
  static async createNotice(noticeData: CreateNoticeData): Promise<NoticeData> {
    try {
      // Validation des données
      if (!noticeData.comment || noticeData.comment.trim().length === 0) {
        throw new NoticeError(
          "Le commentaire de l'avis est requis",
          "NOTICE_COMMENT_REQUIRED",
          400
        );
      }
   ...
   */
  static async getAllNotices(filters: NoticeSearchFilters = {}): Promise<{
    notices: NoticeData[];
    total: number;
    page: number;
    totalPages: number;
    ...
    try {
      ...
      const noticesList: NoticeData[] = notices.map((notice) => ({
   ...
   */
  static async getNoticeById(noticeId: number): Promise<NoticeData | null> {
    try {
  ...
  static async searchNotices(
    ...
    try {
      ...
      throw new NoticeError(
        "Erreur lors de la recherche d'avis",
   ...
   */
  static async noticeExists(noticeId: number): Promise<boolean> {
    try {
      const notice = await Notice.findByPk(noticeId);
      return !!notice;
    } catch (error) {
      throw new NoticeError(
        "Erreur lors de la vérification de l'avis",
        "CHECK_NOTICE_ERROR",
        500
      );
    }
   ...
   */
  static async getRecentNotices(limit: number = 10): Promise<NoticeData[]> {
    try {
      const notices = await Notice.findAll({
        limit: Number(limit),
        order: [["published_at", "DESC"]],
