...
export class ImageService {
    ...
    // Je traite une image uploadee et je cree toutes les tailles
    async processImage(imageBuffer: Buffer, type: 'book' | 'author', id: number, coverId?: number): Promise<string> {
        try {
            // J'utilise coverId si fourni, sinon un timestamp pour les uploads manuels
            ...
            // Je verifie si les fichiers existent deja (cache)
            if (coverId && await this.checkExistingFiles(baseFilename, uploadsDir)) {
                console.log(`Cache hit: Images deja existantes pour ${baseFilename}`)
                return this.buildExistingFilesJson(baseFilename, urlPrefix, type)
            }

            // Je cree les differentes tailles selon le type
            ...
            // Je retourne l'objet JSON avec toutes les tailles
    ...
    // Je verifie si tous les fichiers d'un baseFilename existent
    private async checkExistingFiles(baseFilename: string, uploadsDir: string): Promise<boolean> {
        try {
            const expectedSizes = ['thumb', 'small', 'medium']
    ...
    // Je construis le JSON des fichiers existants
    ...
    // Je supprime les anciennes images (livre ou auteur)
    async deleteImages(imageUrl: string, type: 'book' | 'author'): Promise<void> {
        try {
            if (!imageUrl) { return }

            // Je parse l'objet JSON des URLs
    ...
    // Je valide qu'un fichier est bien une image
    ...
    // J'obtiens les infos d'une image
