...
export class UserRoleService {
  ...
  static async removeRoleFromUser(
    ...
    try {
      ...
      if (result === 0) {
        throw new UserRoleError(
          "Association utilisateur-rôle non trouvée",
          "USER_ROLE_NOT_FOUND",
          404
        );
      }
   ...
   */
  static async getUserRoles(userId: number): Promise<UserRoleData[]> {
    try {
   ...
   */
  static async getRoleUsers(roleId: number): Promise<UserRoleData[]> {
    try {
      const userRoles = await UserRole.findAll({
  ...
  static async replaceUserRoles(
    ...
    try {
      ...
      const newUserRoles: UserRoleData[] = [];
      ...
      throw new UserRoleError(
        "Erreur lors du remplacement des rôles",
        "REPLACE_USER_ROLES_ERROR",
        500
      );
    }
  }
