...
export class AuthorService {
    ...
    static async createAuthor(authorData) {
        try {
            ...
            if (!authorData.lastname || authorData.lastname.trim().length === 0) {
                throw new AuthorError("Le nom de famille est requis", "LASTNAME_REQUIRED", 400);
            }
            ...
            if (existingAuthor) {
                throw new AuthorError("Un auteur avec ce nom existe déjà", "AUTHOR_ALREADY_EXISTS", 409);
            }
            // Créer l'auteur
            ...
            throw new AuthorError("Erreur lors de la création de l'auteur", "CREATE_AUTHOR_ERROR", 500);
        }
    }
    /**
     * Récupérer tous les auteurs avec filtres optionnels
     */
    static async getAllAuthors(filters = {}) {
        try {
            ...
            throw new AuthorError("Erreur lors de la récupération des auteurs", "GET_AUTHORS_ERROR", 500);
        }
    }
    /**
    ...
    static async getAuthorById(authorId) {
        try {
            ...
            throw new AuthorError("Erreur lors de la récupération de l'auteur", "GET_AUTHOR_ERROR", 500);
        }
    }
    /**
    ...
    static async updateAuthor(authorId, updateData) {
        try {
            ...
            if (updateData.lastname !== undefined &&
                ...
                throw new AuthorError("Le nom de famille ne peut pas être vide", "LASTNAME_REQUIRED", 400);
            }
            // Vérifier les doublons si on modifie le nom
            if (updateData.firstname !== undefined ||
                updateData.lastname !== undefined) {
                const newFirstname = updateData.firstname !== undefined
                    ? updateData.firstname?.trim() || null
                    : author.dataValues.firstname;
                const newLastname = updateData.lastname !== undefined
            ...
            throw new AuthorError("Erreur lors de la mise à jour de l'auteur", "UPDATE_AUTHOR_ERROR", 500);
        }
    }
    /**
     * Supprimer un auteur
    ...
    static async deleteAuthor(authorId) {
        try {
            ...
            throw new AuthorError("Erreur lors de la suppression de l'auteur", "DELETE_AUTHOR_ERROR", 500);
        }
    }
    /**
     * Rechercher des auteurs par nom
     */
    static async searchAuthors(searchTerm, limit = 10) {
        try {
            ...
            throw new AuthorError("Erreur lors de la recherche d'auteurs", "SEARCH_AUTHORS_ERROR", 500);
        }
    }
    /**
    ...
    static async getAuthorStats() {
        try {
            ...
            throw new AuthorError("Erreur lors de la récupération des statistiques", "GET_STATS_ERROR", 500);
        }
    }
}
