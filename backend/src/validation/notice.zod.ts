...
import { idParamSchema, paginationSchema, searchQuerySchema, booleanQuerySchema } from './common.zod.js';
  ...
  // Schema de création (POST /notices)
export const createNoticeSchema = z.object({
    ...
    // id_user sera ajouté automatiquement par le middleware auth
});

  // Schema de mise à jour (PUT/PATCH /notices/:id)
export const updateNoticeSchema = createNoticeSchema
    .omit({ id_book: true }) // On ne change pas le livre
    .partial();

  // Schema des paramètres de route (/notices/:id) - Réutilise common
export const noticeParamsSchema = idParamSchema;

  // Schema de recherche (?id_book=123&is_spoiler=false&is_public=true) - Réutilise common
