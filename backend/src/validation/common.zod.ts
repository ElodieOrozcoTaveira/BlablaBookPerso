...
// Schema pour les paramètres d'ID (/:id)
...
export const paginationSchema = z.object({
    ...
    .transform(val => {
        ...
        if (num > 100) {throw new Error('Maximum 100 éléments par page');}
        return num;
    })
});

// Schema pour recherche générale
...
// Schema pour les années (birth_year, publication_year, etc.)
export const yearSchema = z.number()
    .int('L\'année doit être un entier')
    .min(-3000, 'L\'année doit être postérieure à -3000')
    .max(new Date().getFullYear(), 'L\'année ne peut pas être dans le futur');

// Schema pour les années en query params
