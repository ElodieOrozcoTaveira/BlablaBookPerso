...
interface ReadingListAttributes {
    ...
    id_library: number;
    id_book: number;
    id_reading_list_collection?: number; // Nouvelle référence vers une collection nommée
...
class ReadingList extends Model<ReadingListAttributes, ReadingListCreationAttributes> implements ReadingListAttributes {
    ...
    public reading_status!: 'owned' | 'to_read' | 'reading' | 'read' | 'abandoned';
...
ReadingList.init(
{
    ...
    id_reading_list_collection: {
        ...
        allowNull: true,
        references: {
            model: 'READING_LIST_COLLECTION',
            key: 'id_reading_list_collection',
        }
    },
...
},
{
    ...
    indexes: [
        ...
        {
            unique: true,
            fields: ['id_reading_list_collection', 'id_book'],
            where: {
                id_reading_list_collection: {
