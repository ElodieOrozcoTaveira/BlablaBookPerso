...
interface BookAttributes {
    id_book: number;
    isbn?: string;                    // MODIFIÉ: ISBN peut être NULL (Open Library)
    title: string;
    description?: string;
    ...
    cover_url?: string;
    cover_local?: string;
    open_library_key?: string;       // IMPORTANT: Clé unique pour éviter doublons import
    created_at: Date;
    updated_at: Date;
}
...
class Book extends Model<BookAttributes, BookCreationAttributes> implements BookAttributes {
    ...
    public isbn?: string;            // MODIFIÉ: ISBN peut être NULL
    public title!: string;
...
Book.init(
{
    ...
    isbn: {
        ...
        allowNull: true,             // MODIFIÉ: NULL car souvent absent dans Open Library
        unique: true,
        validate: {
            isNumeric: {
            ...
            len: {
                args: [10, 13],
                msg: 'ISBN doit faire entre 10 et 13 caractères'
            }
        }
    },
    ...
    language: {
        ...
        allowNull: true,
        defaultValue: 'en',          // MODIFIÉ: 'en' par défaut (Open Library = 'eng')
    },
