...
const transformDates = (obj: any, visited = new WeakSet()): any => {
    ...
    for (const [key, value] of Object.entries(obj)) {
        // Ignorer les propriétés internes de Sequelize qui causent des boucles
...
export const dateFormatterMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.json = function(data: any) {
        try {
            // Utiliser JSON.stringify avec replacer pour éliminer les références circulaires
            const cleanData = JSON.parse(JSON.stringify(data, (key, value) => {
                // Ignorer les propriétés Sequelize internes qui causent des problèmes
                if (key.startsWith('_') || 
                    key === 'parent' || 
                    key === 'through' || 
                    key === 'uniqno' || 
            ...
            // Transforme toutes les dates sur les données nettoyées
            const transformedData = transformDates(cleanData);
            return originalJson(transformedData);
        } catch (error) {
            // En cas d'erreur, renvoyer la donnée originale sans transformation
