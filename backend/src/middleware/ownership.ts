...
/**
 * Middleware generique pour verifier la propriete d'une ressource.
 * - Charge la ressource via findByPk.
 * - 404 si absente.
 * - 403 si non proprietaire (et pas admin).
...
export function requireOwnership(opts: OwnershipOptions) {
  ...
  // Support de surcharge modele pour tests unitaires (injection __TEST_MODEL_OVERRIDES)
  ...
  if (resourceLoader) {
    ...
    // Appel direct de la fonction mockee pour conserver son contexte
