...
async function getUserPermissions(userId: number): Promise<string[]> {
    ...
    const user = await UserModel.findByPk(userId);
        ...
        // Utiliser les méthodes magiques Sequelize (générées par belongsToMany)
...
/**
 * Middleware de vérification des permissions RBAC avec audit Zero Trust
 */
export function requirePermission(requiredPermissions: string | string[]) {
    ...
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            // Bypass total pour tests unitaires ciblés (éviter setup session / rôles)
            ...
            // 2. Récupération des permissions utilisateur
            const userPermissions = await getUserPermissions(userId);
            
            // 3. Vérification des permissions (mode AND - toutes requises)
...
// Middleware d'exemple pour différents niveaux d'accès
