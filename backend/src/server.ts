...
const PORT = Number(process.env.PORT) || 3000;
  ...
  const startServer = async () => {
    try {
      ...
      // Synchronisation des tables (creation si necessaire) suite probleme front
      console.log(' Synchronisation des tables Sequelize...');
      await sequelize.sync({ alter: false });
      console.log(' Tables Sequelize synchronisees !');
 ...
 app.use(sessionMiddleware);
  ...
  app.use(conditionalCsrfProtection);
    ...
    // 5. IMAGES STATIQUES (avec headers de cache optimaux)
    app.use('/uploads', express.static('uploads', {
      maxAge: '30d', // Cache 30 jours (compromis performance/flexibilite)
      etag: true,    // ETags pour validation cache
      lastModified: true,
      cacheControl: true,
      setHeaders: (res, path) => {
        // Headers optimaux pour images WebP
