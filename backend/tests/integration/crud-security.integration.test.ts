...
describe('CRUD Security Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        // Create basic data needed for tests (author with ID 1, genre with ID 1)
        await sequelize.query(`INSERT INTO "AUTHOR" (name, created_at, updated_at) VALUES ('Test Author', NOW(), NOW());`);
        await sequelize.query(`INSERT INTO "GENRE" (name, created_at, updated_at) VALUES ('Test Genre', NOW(), NOW());`);
    ...
    describe('Security Tests - Unauthenticated Requests', () => {
        protectedRoutes.forEach(route => {
            describe(`${route.name} Security`, () => {
                // Books creation (POST) est publique pour enrichir la DB, mais pas les autres opérations
                if (route.name !== 'Books') {
