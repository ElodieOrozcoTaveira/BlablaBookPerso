...
vi.mock('../../../src/models/ReadingList.js', () => ({ default: { findOne: vi.fn(), create: vi.fn(), findByPk: vi.fn() } }));
...
describe('Reading List Controller - lazy import', () => {
  ...
  const mockTempBook = { id_book: 777, import_status: 'temporary', update: vi.fn().mockResolvedValue(true) };
  const mockConfirmedBook = { id_book: 888, import_status: 'confirmed', update: vi.fn() };
  ...
  it('importe et confirme automatiquement un livre via open_library_key', async () => {
    req.body = { id_library: 10, open_library_key: '/works/OL123W', reading_status: 'to_read' };
    (Book.findByPk as MockedFunction<any>).mockResolvedValue(mockTempBook);
    const createdEntry = { id_reading_list: 1, id_library: 10, id_book: 777, reading_status: 'to_read', added_at: new Date() };
    ...
    expect(svcInstance.prepareBookForAction).toHaveBeenCalledWith('/works/OL123W', 42, 'add_to_reading_list');
    expect(mockTempBook.update).toHaveBeenCalledWith({ import_status: 'confirmed', imported_reason: 'library' });
    expect(ReadingList.create).toHaveBeenCalledWith(expect.objectContaining({ id_book: 777 }));
    expect(res.status).toHaveBeenCalledWith(201);
  });

  it('retourne 400 si aucun identifiant resolu', async () => {
    req.body = { id_library: 10, reading_status: 'to_read' }; // ni id_book ni open_library_key
    await readingListController.addBookToReadingList(req, res, next);
    expect(res.status).toHaveBeenCalledWith(400);
  });

  it('retourne 404 si import OpenLibrary echoue', async () => {
    req.body = { id_library: 10, open_library_key: '/works/FAIL', reading_status: 'to_read' };
