...
import BookGenre from '../../src/models/BookGenre';
...
describe('Modeles Sequelize', () => {
    ...
    describe('Modele Author', () => {
        ...
        it('devrait echouer si death_date < birth_date', async () => {
            ...
            })).rejects.toThrow('La date de décès ne peut pas être antérieure à la date de naissance');
        });
    ...
    describe('Modèle Genre', () => {
        it('devrait créer un genre valide', async () => {
        ...
        it('devrait échouer avec un nom dupliqué', async () => {
            await Genre.create({
                name: 'Fantasy',
            ...
            await expect(Genre.create({
                name: 'Fantasy', // Nom dupliqué
                description: 'Deuxième genre'
            })).rejects.toThrow();
        });
    });

    describe('Modèle Rate', () => {
        ...
        it('devrait échouer avec une note invalide', async () => {
        ...
        it('devrait échouer si un utilisateur note deux fois le même livre', async () => {
            await Rate.create({
                id_user: user.id_user,
                id_book: book.id_book,
                rating: 4
