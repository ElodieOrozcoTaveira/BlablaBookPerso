...
describe('RateService lazy import (open_library_key)', () => {
  ...
  it('import and confirm if import_status=temporary', async () => {
  ...
  it('does not confirm if already confirmed', async () => {
    const updateMock = vi.fn();
    prepareMock.mockResolvedValue({
      book: { id_book: 12, import_status: 'confirmed', update: updateMock },
      wasImported: false,
      canRollback: false
    });
  ...
  it('conflict if rate already exists', async () => {
    const updateMock = vi.fn();
    prepareMock.mockResolvedValue({
      book: { id_book: 13, import_status: 'confirmed', update: updateMock },
      wasImported: false,
      canRollback: false
    });
  ...
  it('error OPENLIB_IMPORT_FAILED if prepare fails', async () => {
  ...
  it('does not call prepare if id_book provided', async () => {
