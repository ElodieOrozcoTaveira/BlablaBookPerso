...
describe('RateService', () => {
  ...
  it('createRate triggers lazy import when open_library_key provided', async () => {
    // No id_book provided, we mock prepareBookForAction to return a created book (with update)
    const prepResult = {
      wasImported: true,
      book: {
        id_book: 77,
        import_status: 'temporary',
        update: vi.fn(async function (this: any, data: any) {
          Object.assign(this, data);
        })
      }
    } as any;
    const spy = vi.spyOn(
      BookActionModule.BookActionService.prototype,
      'prepareBookForAction'
    ).mockResolvedValue(prepResult);
    (Rate.findOne as any).mockResolvedValue(null);
    (Rate.create as any).mockResolvedValue({
      id_rate: 9,
      id_book: 77,
      rating: 4,
      id_user: userId
    });
    const { rate, error } = await service.createRate(
      { open_library_key: '/works/OL12345W', rating: 4 } as any,
      userId
    );
    expect(error).toBeUndefined();
    expect(rate?.id_book).toBe(77);
    expect(spy).toHaveBeenCalledWith('/works/OL12345W', userId, 'add_rate');
    expect(prepResult.book.update).toHaveBeenCalledWith({
