...
describe('RBAC minimal Users (ADMIN_USERS & VIEW_USER_STATS)', () => {
    ...
    beforeAll(() => {
        ...
        // Inject session (simulate authenticateToken)
        ...
        // Route GET /users protected by ADMIN_USERS
        ...
        // Route GET /users/:id/stats protected by VIEW_USER_STATS
    ...
    // --- /users (ADMIN_USERS) ---
    it('GET /users -> 401 if not authenticated', async () => {
    ...
    it('GET /users -> 403 without ADMIN_USERS permission', async () => {
    ...
    it('GET /users -> 200 with ADMIN_USERS', async () => {
    ...
    it('GET /users/:id/stats -> 401 if not authenticated', async () => {
        (globalThis as any).__RBAC_TEST_SESSION = undefined;
        findByPkMock.mockReset();
    ...
    it('GET /users/:id/stats -> 403 without VIEW_USER_STATS', async () => {
        (globalThis as any).__RBAC_TEST_SESSION = { isAuthenticated: true, userId: 777, email: 'user@test.com' };
    ...
    it('GET /users/:id/stats -> 200 with VIEW_USER_STATS', async () => {
        (globalThis as any).__RBAC_TEST_SESSION = { isAuthenticated: true, userId: 777, email: 'admin@test.com' };
        findByPkMock.mockResolvedValue(userWith(['ADMIN_USERS', 'VIEW_USER_STATS']));
        const res = await request(app).get('/users/42/stats');
        expect(res.status).toBe(200);
