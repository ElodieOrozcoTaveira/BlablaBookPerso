...
describe('RBAC route minimal POST /books (CREATE_BOOK)', () => {
    ...
    // 403 if authenticated without CREATE_BOOK
    it('403 si authentifie sans CREATE_BOOK', async () => {
        (globalThis as any).__RBAC_TEST_SESSION = {
    ...
    // 201 if authenticated with CREATE_BOOK
    it('201 si authentifie avec CREATE_BOOK', async () => {
        (globalThis as any).__RBAC_TEST_SESSION = {
            isAuthenticated: true,
            userId: 1002,
