...
// Mock findByPk via override (no real DB query here)
...
describe('RBAC route minimal POST /authors (CREATE_AUTHOR)', () => {
    ...
    beforeAll(() => {
        ...
        // Minimal route reproducing the chain: (inject session) -> requirePermission('CREATE_AUTHOR') -> handler
        app.post(
            '/authors',
            // inject session from global variable (simulates real authenticateToken)
            (req: any, _res, next) => {
    ...
    it('401 if not authenticated (no session)', async () => {
    ...
    it('403 if authenticated without permission CREATE_AUTHOR', async () => {
    ...
    it('201 if authenticated with CREATE_AUTHOR', async () => {
