...
// mock findByPk via override global (evite hoisting vi.mock + import circulaire)
...
describe('middleware requirePermission (RBAC)', () => {
  ...
  // session de test injectee avant le middleware
  let testSession: any;
  ...
  function mountRoute(
    mw: any,
    handler = (req: any, res: any) => res.json({ ok: true, granted: req.userPermissions })
  ) {
    app.post(
      '/test',
      (req: any, _res, next) => { // inject session avant middleware
        if (testSession) {
  ...
  it('401 si pas authentifie', async () => {
  ...
  it('403 si authentifie mais permission absente', async () => {
  ...
  it('200 si permission unique presente', async () => {
  ...
  it('200 si multi-permissions toutes presentes', async () => {
