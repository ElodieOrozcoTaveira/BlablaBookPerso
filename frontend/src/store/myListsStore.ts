...
interface ReadingList {
  ...
  created_at: string;
  updated_at: string;
  books: Book[]; // 👈 chaque liste contient un tableau de livres
}
...
interface MyListsState {
  ...
  // ➕ Ajouter un livre dans une liste
  // eslint-disable-next-line no-unused-vars
  addBookToList: (listId: number, book: Book) => void;
}
...
export const useMyListsStore = create<MyListsState>((set, get) => ({
  ...
  // ➕ Ajouter un livre à une liste locale
  addBookToList: (listId: number, book: Book) => {
    set((state) => ({
      lists: state.lists.map((list) =>
        list.id_library === listId
          ? {
              ...list,
