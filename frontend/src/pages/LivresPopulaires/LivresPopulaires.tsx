...
export default function PopularBooks() {
  ...
  useEffect(() => {
    const fetchPopularBooks = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:3000/api/openlibrary/search/books?query=${encodeURIComponent(randomTerm)}`);
        const allBooks: Book[] = response.data.data || [];
        const filteredBooks = allBooks.filter(b => b.title && b.title.length < 40);
  ...
  return (
    <section className="popular-books-grid" aria-label="Liste des livres populaires">
      {popularBooks.map((book) => (
        <div
          key={book.id?.toString() || book.isbn || book.open_library_key || `book-${book.title}`}
        ...
        >
          {book.cover_url ? (
            ...
            <div className="popular-books-grid__no-cover">Pas de couverture</div>
          )}
          <div className="popular-books-grid__title">{book.title}</div>
          <div className="popular-books-grid__authors">{book.authors || 'Auteur inconnu'}</div>
          <button
